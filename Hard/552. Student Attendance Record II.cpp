...
public:
    int checkRecord(int n) {
        ...
        // Sum up the counts for all combinations with length "n" with different absent and lates counts
        int count = 0;
        for (int totalAbsences = 0; totalAbsences <= 1; ++totalAbsences) {
            for (int consecutivesLates = 0; consecutivesLates <= 2; ++consecutivesLates) {
                count = (count + dp[n][totalAbsences][consecutivesLates]) % MOD;
            }
        }
        return count;
    }
